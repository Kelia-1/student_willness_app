  ...
  Future<void> sendMessage(ChatMessage message) async {
    try {
      ...
      await _firestore.collection('chats').doc(chatId).set({
        'participants': [message.senderId, message.peerId],
        'lastActive': FieldValue.serverTimestamp(),  // <-- Updated here
  ...
  Future<List<ChatPeer>> getChatPeers(String userId) async {
    try {
      final chats = await _firestore
          ...
          .orderBy('lastActive', descending: true) // order by lastActive for recent chats first
          .get();
      ...
      for (final chat in chats.docs) {
        ...
        if (peerProfile != null) {
          final lastActiveTimestamp = chat.data()['lastActive'] as Timestamp?;
          final lastActive = lastActiveTimestamp?.toDate() ?? DateTime.now();
          ...
          peers.add(ChatPeer(
            ...
            lastActive: lastActive,  // <-- Use lastActive here
